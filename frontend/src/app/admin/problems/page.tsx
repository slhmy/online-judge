...
const problemSchema = z.object({
  ...
  time_limit: z.coerce
    ...
    .max(60, "Time limit cannot exceed 60 seconds"),
  memory_limit: z.coerce
    .number()
    .min(256, "Memory limit must be at least 256 KB")
    .max(524288, "Memory limit cannot exceed 512 MB"),
...
export default function AdminProblemsPage() {
  ...
  useEffect(() => {
    if (editingProblem) {
      ...
      setValue(
        "difficulty",
        editingProblem.difficulty as "easy" | "medium" | "hard",
      );
    ...
    if (statementFormat === "pdf") {
      return (
        <div className="text-sm text-yellow-700 dark:text-yellow-300">
          PDF format is stored as content metadata. Inline PDF preview is not
  ...
  const upsertProblemStatement = async (problemID: string) => {
    const res = await fetch(
      `${BFF_URL}/api/v1/problems/${problemID}/statement`,
      {
        method: "PUT",
        credentials: "include",
        headers: {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const statementRes = await fetch(
          `${BFF_URL}/api/v1/problems/${problem.id}/statement`,
          {
            credentials: "include",
          },
        );
        if (statementRes.ok) {
          ...
          if (typeof statementData === "string") {
            ...
            statementData &&
            typeof statementData.content === "string"
          ) {
            setStatementContent(statementData.content);
            const fmt = (statementData.format || "markdown") as
              | "markdown"
              | "html"
              | "plain"
              | "pdf";
      ...
      {showForm && (
        <div className="bg-card rounded-xl shadow p-6 mb-8">
          <h2 className="text-lg font-semibold mb-4 text-foreground">
            {editingProblem
              ? `Edit Problem: ${editingProblem.name}`
              : "Create New Problem"}
          </h2>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-3">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setStatementFormat(
                        e.target.value as "markdown" | "html" | "plain" | "pdf",
                      )
                    }
                    className="w-full px-3 py-2 border border-border rounded-xl bg-white text-foreground"
                  >
              ...
              <div
                className={`grid gap-3 ${previewMode === "split" ? "grid-cols-1 md:grid-cols-2" : "grid-cols-1"}`}
              >
                {previewMode !== "preview" && (
              ...
              <TableBody>
                {problems.map((problem) => (
                  <TableRow key={problem.id}>
                    ...
                    <TableCell>{problem.time_limit}s</TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      <div className="flex flex-wrap gap-2">
                        ...
                        <Button
                          ...
                          onClick={() =>
                            router.push(
                              `/admin/problems/${problem.id}/testcases`,
                            )
                          }
                        >
                          Test Cases
